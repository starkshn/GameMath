...
void SoftRenderer::LoadScene2D()
{
    ...
    // 플레이어의 생성과 설정
    constexpr float playerScale = 50.f;
...
void SoftRenderer::Update2D(float InDeltaSeconds)
{
    ...
    for (auto it = g.SceneBegin(); it != g.SceneEnd(); ++it)
    {
        ...
        {
            TransformComponent& t = go.GetTransform();
            Math::GetSinCos(_s, _c, De);
            t.AddPosition(Vector2(_c, 0.f) * moveSpeed * InDeltaSeconds);
            t.AddRotation(rotateSpeed * InDeltaSeconds);
            // t.SetScale(Vector2::One * De);
        }
    }
}
...
void SoftRenderer::Render2D()
{
    ...
    for (auto it = g.SceneBegin(); it != g.SceneEnd(); ++it)
    {
        ...
        if (gameObject == PlayerGo)
        {
            ...
            r.PushStatisticText("De : " + std::to_string(De));
        }
    }
}
